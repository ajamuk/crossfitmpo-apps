...
// ---------------------------------------------------------------------------
// API (fetch contra el backend Express)
// ---------------------------------------------------------------------------
const api = {
  async crearUsuario(nombre_usuario, box) {
    const r = await fetch('/api/usuarios', {
      ...
      body: JSON.stringify({ nombre_usuario, box }),
    });
  ...
  async entrenamientos(categoria = '') {
    ...
    const r = await fetch(`/api/entrenamientos${q}`);
    return r.json();
  },
  async registros(usuarioId) {
    const r = await fetch(`/api/registros/${usuarioId}`);
    return r.json();
  },
  ...
  async ranking(box = '') {
    ...
    const r = await fetch(`/api/ranking${q}`);
    return r.json();
  },
};
